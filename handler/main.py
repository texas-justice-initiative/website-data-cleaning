...
def main():
    ...
    master_2017: pd.DataFrame = get_s3_data(
        "tji-static-datasets", "reformatted_cdr_2017_master_file.csv"
    )

    agency_county: pd.DataFrame = get_s3_data(
        "tji-static-datasets", "agencies_and_counties.csv"
    )
    ...
    for range_name in RANGES:
        ...
        sheet_data: Optional[Tuple[list, list]] = retrieve_sheet(
            config.get("sheets", {}), doc_config
        )
    ...
    LOGGER.debug(
        f"CDR data downloaded from Google Sheets. Dataframe shape: {cdr.shape}"
    )
    ...
    LOGGER.debug(
        "Changed %d cols to datetime (from some other dtype):" % len(different),
        different,
    )
    ...
    cdr["death_date"] = pd.to_datetime(
        cdr.death_date_and_time.apply(
            lambda dt: datetime.date(dt.year, dt.month, dt.day)
        )
    )
    ...
    cdr.loc[cdr.ethnicity_other.astype(str) == "0", "ethnicity_other"] = (
        None  # 2005 form has description field for "OTHER"
    )

    other_eth = cdr.ethnicity == "OTHER"
    LOGGER.info(
        'Merging %d "ethnicity_other" values into the main "ethnicity" column'
        % other_eth.sum()
    )
    ...
    county_lookup = cdr["agency_name"].apply(
        lambda name: agency_county.get(name, np.nan)
    )
    cdr["agency_county"] = county_lookup.fillna(cdr["agency_county"])

    # Manually handle one major agency
    cdr.loc[cdr["agency_name"] == "TEXAS DEPT OF CRIMINAL JUSTICE", "agency_county"] = (
        "STATE"
    )
    ...
    LOGGER.info(
        "Dropping %d records from 'TEST CDR AGENCY', leaving %d records"
        % (test_agencies.sum(), len(cdr))
    )

    ### death details (lots of columns here)
    cdr["death_location"] = np.where(
        cdr["death_location"].isin([0, 1]), np.nan, cdr["death_location"]
    )
    ...
    cdr["death_location"] = cdr["death_location"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )
    ...
    cdr["means_of_death"] = cdr["means_of_death"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )

    cdr[cdr.means_of_death == OTHER_SPECIFY][
    ...
    cdr["manner_of_death"] = cdr["manner_of_death"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )
    ...
    cdr.loc[indices, "manner_of_death_description"] = cdr.loc[
        indices, "manner_of_death"
    ]
    cdr.loc[indices, "manner_of_death"] = OTHER_SPECIFY

    frame = cdr[
        (cdr.manner_of_death == "SUICIDE")
        & (cdr.means_of_death != "HANGING, STRANGULATION")
    ]
    frame = frame[
        (frame.medical_cause_of_death.fillna("").str.contains("HANGING"))
    ...
    frame = cdr[
        (cdr.manner_of_death == "SUICIDE") & (cdr.means_of_death == "NOT APPLICABLE")
    ]
    ...
    assert (
        len(
            cdr[
                (cdr.manner_of_death == "SUICIDE")
                & (cdr.means_of_death == "NOT APPLICABLE")
            ]
        )
        == 0
    )
    ...
    cdr["pre_existing_medical_condition"] = cdr["pre_existing_medical_condition"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )
    ...
    * 2016 form: "If the death was an accident, homicide **or suicide**, who caused the death?" (emphasis added)
    ...
    cdr["who_caused_the_death"] = cdr["who_caused_the_death"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )
    cdr.loc[
        ~cdr.manner_of_death.isin(["HOMICIDE", "ACCIDENTAL", OTHER_SPECIFY]),
        "who_caused_the_death",
    ] = "NOT APPLICABLE"

    cdr["medical_examinor_coroner_evalution"] = cdr[
    ...
    cdr["were_the_charges"] = cdr["were_the_charges"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )
    ...
    cdr["type_of_custody"] = cdr["type_of_custody"].apply(
        lambda x: None if pd.isnull(x) else replacements.get(x.strip(), x)
    )
    ...
    cdr["specific_type_of_custody_facility"] = cdr[
        "specific_type_of_custody_facility"
    ].apply(lambda x: x if pd.isnull(x) else replacements.get(x.strip(), x))
    ...
    LOGGER.debug(
        "Dropping %d rows that are 100%% duplicates of another row, leaving %d rows"
        % (pure_dupes.sum(), len(cdr))
    )
    ...
    for i, dr in enumerate(dedup_rounds):
        LOGGER.debug(
            "**** Dedup step %d: find duplicates on these columns:" % (i + 1), dr
        )
        ...
        LOGGER.debug(
            "Dropping %d duplicates and adding %d merged records, yielding %d records"
            % (dropping, len(mr), len(new_cdr))
        )

    LOGGER.debug(
        "Ultimately removing %d duplicate records, leaving %d"
    ...
    for rd, cols in enumerate(dedup_rounds):
        LOGGER.debug(
            "In round %d, there were %d record merges based on"
            % (rd, len(all_merged_records[rd])),
            cols,
        )
    ...
    def fix_century(
        df: pd.DataFrame, check_columns: list, compare_column: str
    ) -> pd.DataFrame:
        """
        ...
        for col in check_columns:
            ...
            df.loc[df[compare_column] + datetime.timedelta(days=1) < df[col], col] = df[
                col
    ...
    LOGGER.debug(
        "For %d records with death date before custoday date, setting the days_from_custody_to_death to NaN"
        % (delta.dt.days < -1).sum()
    )
